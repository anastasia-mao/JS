// старый вариант, сейчас нужен для корзины
var testjson = '[ \
...
// отрисовка каталога шаблонизатором
function catalog(){
    template = "<h2 id=\"catalogHead\" class=\"catalogHead\">Каталог</h2>\
        {{#test}}<div id=\"catalogItemId{{key}}\" class=\"catalogItem\">\
        <img class=\"catalogImg\" src=\"{{itemImg}}\">\
        <h3 class=\"catalogHeader\">{{itemName}}</h3>\
        <h5 class=\"catalogPrice\">{{price}}</h5>\
        <button id=\"add{{key}}\" onclick=\"basket.add('{{key}}',1); basket.clear(); basket.render();\">Добавить</button></div>{{/test}}";
        
    result = Mustache.render(template, data);
    var dest = document.getElementById('catalog');
